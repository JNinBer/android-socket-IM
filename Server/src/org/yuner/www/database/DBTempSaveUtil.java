...
public class DBTempSaveUtil {
	...
	public static void saveUnsentChatMsg(int senderId, int receiverId, ChatEntity ent0) {
		String toInsert = ent0.toString();
		insertItem(senderId, receiverId, toInsert, GlobalMsgTypes.msgFromFriend);
	}
	...
	private static void insertItem(int senderId, int receiverId, String str0, int type0) {
		...
		try {
			...
			str0 = UnicodeUtil.GB2312toutf8(str0);
		...
		try {
			...
			if(rs.first()) {
				do {
					...
					str0 = UnicodeUtil.utf8toGB2312(str0);
					outList.add(str0);
				} while (rs.next());
			}
