...
public class ClientSendThread extends Thread {
	...
	private static final int mWaitPerRound = 1;
	private static final int mWaitHowManyRounds = 6000;
		...
		while(mIsRunning) {
			if(mSendList.size() == 0) {
				...
				while(!mReceived) {
					...
					waitingTime ++;
					if(waitingTime >= mWaitHowManyRounds) {
	...
	public void saveUnsendMsgs() {
		for(SendStackItem item0 : mSendList) {
			int type = item0.getType();
			String str = item0.getStr();
			int senderId = GlobalInts.idPlaceholder;   // because senderId is useless
			int receiverId = mClientActivity.getUserInfo().getId();
			switch(type) {
				case GlobalMsgTypes.msgFromFriend:
					DBTempSaveUtil.saveUnsentChatMsg(senderId, receiverId, ChatEntity.Str2Ent(str));
					break;
				case GlobalMsgTypes.msgFriendshipRequest:
					DBTempSaveUtil.saveUnsentFrdReqs(senderId, receiverId, str);
					break;
				case GlobalMsgTypes.msgFriendshipRequestResponse:
					DBTempSaveUtil.saveUnsentFrdReqResponse(senderId, receiverId, str);
					break;
				default:
					System.out.println("fuck, error happend");
					break;
			}
		}
	}
